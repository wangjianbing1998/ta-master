...
def main(data_dir='BigData', result_dir='processed_transformer_dir', num_per_xlsx=10):
    with_na_ = result_dir + '/_with_na'
    os.makedirs(with_na_, exist_ok=True)
    without_na_ = result_dir + '/_without_na'
    os.makedirs(without_na_, exist_ok=True)
    without_na_removed_outliner = result_dir + '/_without_na_removed_outliner'
    os.makedirs(without_na_removed_outliner, exist_ok=True)
    ...
    res_code = '-'.join(res['code'].value_counts().keys().to_list())
    res.to_excel(os.path.join(with_na_, f'{res_code}.xlsx'), index=False)
    res.dropna(inplace=True)
    res.to_excel(os.path.join(without_na_, f'{res_code}.xlsx'), index=False)
    print(res)
    change_df = outliner_remover.remove_max_min_by_pct_change(res, 20, 200, 1.2)
    change_df.to_excel(os.path.join(without_na_removed_outliner, f'{res_code}.xlsx'), index=False)
